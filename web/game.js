...
// ---- entities ----
const player = { x: LEVEL.playerStart[0], y: LEVEL.playerStart[1], w: 18, h: 22 };
const zombies = LEVEL.zombies.map(([x, y]) => mkZombie(x, y));
function mkZombie(x, y) { return { x, y, w: 18, h: 22, vx: 0, vy: 0, t: 0, hp: ZHP, flash: 0 }; }
...
let health = 100, lives = 3, totalCoins = 0, kills = 0, state = 'play', stamina = STAM_MAX, animClock = 0;
...
// ---- input ----
const keys = {};
addEventListener('keydown', e => { keys[e.code] = true; if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Space'].includes(e.code)) e.preventDefault(); if (state !== 'play' && e.code === 'KeyR') location.reload(); });
...
function hurt(dmg) {
  if (invuln > 0) return;
  health -= dmg; hurtFlash = Math.min(1, hurtFlash + 0.35); shakeT = 0.16; invuln = 0.7;
  if (health <= 0) { lives--; if (lives <= 0) { state = 'gameover'; return; } health = 100; player.x = LEVEL.playerStart[0]; player.y = LEVEL.playerStart[1]; invuln = 1.2; }
}
function fire(dx, dy) {
...
function update(dt) {
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    for (let j = zombies.length - 1; j >= 0; j--) { const z = zombies[j]; if (b.x >= z.x && b.x <= z.x + z.w && b.y >= z.y && b.y <= z.y + z.h) {
      ...
      if (z.hp <= 0) { spawnBurst(z.x + 9, z.y + 11, '#8ec257', 16, 170, 3); spawnBurst(z.x + 9, z.y + 11, '#ffffff', 6, 120, 2); dropLoot(z); zombies.splice(j, 1); kills++; }
  ...
  // pickups
  coins = coins.filter(co => { if (aabb(player.x, player.y, player.w, player.h, co.x - 10, co.y - 10, 28, 28)) { totalCoins++; spawnBurst(co.x, co.y, '#ffd23f', 8, 80, 2); if (totalCoins >= WINCOINS) state = 'win'; return false; } return true; });
  ammoCrates = ammoCrates.filter(a => { if (wAmmo[curW] < WEAPONS[curW].mag && aabb(player.x, player.y, player.w, player.h, a.x, a.y, a.w, a.h)) { wAmmo[curW] = WEAPONS[curW].mag; spawnBurst(a.x + 10, a.y + 8, '#ffd23f', 8, 90, 2); showToast('+ набої'); return false; } return true; });
  for (let i = loot.length - 1; i >= 0; i--) { const a = loot[i]; if (aabb(player.x, player.y, player.w, player.h, a.x, a.y, a.w, a.h)) {
    ...
    spawnBurst(a.x + 11, a.y + 9, WEAPONS[a.weapon].color, 14, 130, 3);
...
function drawHUD() {
  ...
  // health
  ctx.strokeStyle = '#7a5'; ctx.lineWidth = 2; ctx.strokeRect(14, 9, 190, 16);
  ctx.fillStyle = health > 60 ? '#4caf50' : health > 30 ? '#ffa726' : '#ef5350'; ctx.fillRect(14, 10, health / 100 * 188, 14);
  // stamina
  ctx.strokeStyle = '#7a5'; ctx.strokeRect(14, 27, 190, 6); ctx.fillStyle = stamina > 25 ? '#42a5f5' : '#ab47bc'; ctx.fillRect(14, 28, stamina / 100 * 188, 4);
  ctx.textBaseline = 'middle'; ctx.fillStyle = '#5a4a32'; ctx.font = 'bold 18px Calibri,sans-serif';
  ctx.fillText('♥' + lives, 214, 14);
  ...
  ctx.textAlign = 'right'; ctx.font = 'bold 20px Calibri,sans-serif'; ctx.fillStyle = '#c79a1a'; ctx.fillText('★ ' + totalCoins + '/' + WINCOINS, VIEW_W - 14, 13);
  ctx.font = 'bold 14px Calibri,sans-serif'; ctx.fillStyle = '#6a8f3a'; ctx.fillText('☠ ' + kills, VIEW_W - 14, 30); ctx.textAlign = 'left';
